...
{
    unsigned char mnemonic_num = mem[0];
    const char* mnemonic = mnemonics[mnemonic_num];
    ...
    if (mnemonic[4] == 'A' || mnemonic[4] == 'i') {
        ...
        sprintf(out_buffer64, "OPCODE=%02X[%s] OPERAND=%s %s", mnemonic_num, mnemonic, high_bit, low_bit);
        ...
        sprintf(out_buffer64, "OPCODE=%02X[%s] OPERAND=%02X %02X", mnemonic_num, mnemonic, high_bit, low_bit);

        ++mem;
    } else {
        unsigned char low_bit = *++mem;

        sprintf(out_buffer64, "OPCODE=%02X[%s] OPERAND=.. %02X", mnemonic_num, mnemonic, low_bit);
